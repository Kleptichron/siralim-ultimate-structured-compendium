...
// diluting the trait facets.
{
  ...
  // Traits the game has but the community sheet does not are enemy-only boss and
  // False God traits. They carry no creature, family or material, so they cannot
  // occupy a build slot and would arrive untagged. Patch 3.0 exposes the database
  // files, which is the point to bring them in properly rather than tag a few
  // hundred records that a content patch is about to rewrite.
  let enemyOnlySkipped = 0;
  for (const g of gameTraits) {
    const c = pairs.get(g);
    if (!c) { enemyOnlySkipped++; continue; }
  ...
  if (enemyOnlySkipped) notes.push(`${enemyOnlySkipped} enemy-only traits in the game are not imported (awaiting 3.0)`);
  for (const c of unmatchedCommunity) {
...
// Artifact modifiers, spell gem properties and god blessings are all present in
// source/game/ and all import cleanly, but none of them are tagged yet. Patch 3.0
// exposes the database files and changes a lot of text, so they wait for that
// rather than landing as ~700 untagged records.
